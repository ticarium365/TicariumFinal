...
### Sprint 50 (Adapter İskeleti + Excel Export + Sync Logs) — TAMAMLANDI

- Yeni tablolar: `channel_credentials` (companyId+channelKey unique, mode=test|live, credentials JSONB, lastSyncAt/Status), `channel_sync_logs` (her push/pull için kayıt — operation, status, mode, requestPayload, responsePayload, errorMessage, durationMs).
- Adapter framework: `artifacts/api-server/src/services/channels/` altında `BaseChannelAdapter` (test modunda dry-run otomatik, live modunda alt sınıf HTTP gövdesi sağlayacak) + 4 stub: `TrendyolAdapter`, `HepsiburadaAdapter`, `N11Adapter`, `AmazonTrAdapter` — her biri kendi pazaryeri payload formatını (Trendyol: barcode+productMainId, Hepsiburada: merchantSku, N11: productSellerCode+stockItems, Amazon: SKU+EAN+marketplace) inşa eder.
- Yeni endpoint'ler: `GET/PUT /api/channels/:key/credentials` (kimlik bilgileri yönetimi — gizli alanlar maskelenir, masked değerle gelirse mevcut korunur), `POST /api/channels/:key/sync` (aktif tüm listeleri adapter üzerinden gönderir, test modunda dry-run + her sonucu `channel_sync_logs`'a yazar), `GET /api/channels/:key/logs` (son 100 kayıt), `GET /api/channels/:key/export.xlsx` (Trendyol benzeri standart şablonda Excel — pazaryeri panellerinde manuel toplu yükleme için).
- Frontend: Kanal detay sayfasına başlık üstü buton barı (Excel İndir, Bağlantı, Geçmiş, Senkronize Et), TEST MODU/CANLI rozetleri, son senkron durumu rozeti, kimlik bilgileri dialog'u (her kanal için özel alanlar), senkron geçmişi tablosu (zaman/işlem/ürün/durum/mod/süre/hata).
- Güvenlik: Tüm endpoint'lerde `req.companyId!` kullanımı (önceki kod yanlış `req.session.companyId` kullanıyordu — düzeltildi); secret alanlar (`/secret|key|token|password|api/i`) maskeli döner; cross-tenant izolasyon doğrulandı.
- E2E: PROSAN için Trendyol kimlik bilgileri kaydedildi → maskeli geri döndü → sync test modunda 9/9 başarılı (dry-run, gerçek API'a gönderilmedi) → loglar Trendyol payload formatında kaydedildi → Excel indirildi (15.7 KB, geçerli xlsx). NİHAT, PROSAN'ın kimlik bilgilerini/loglarını göremiyor.
- **Live mode**: Adapter'lar şu anda live modunda `"Live mode not yet implemented"` hatası döner. Gerçek API kimlik bilgileri geldiğinde her adapter'ın `pushListing/updateStock/updatePrice` metodları HTTP çağrısı yapacak şekilde tamamlanacak (payload yapısı zaten doğru — sadece auth + endpoint URL + response handling eklenecek).
