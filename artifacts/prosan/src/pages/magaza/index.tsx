...
const STATUS_BADGE: Record<Storefront["status"], { label: string; cls: string }> = {
  ...
  active: { label: "Yayında", cls: "bg-emerald-100 text-emerald-800" },
  paused: { label: "Durduruldu", cls: "bg-amber-100 text-amber-800" },
};
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Card className="border-l-4 border-l-emerald-500">
          <CardHeader>
            ...
            <CardDescription>
              <code className="text-xs bg-emerald-50 px-1 rounded">firmaadi.ticarium365.shop</code> alt alanında bizim hazır temayla mağaza. Sıfır geliştirme.
            </CardDescription>
          </CardHeader>
        </Card>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {items.map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center gap-4 p-4 border rounded-lg hover:bg-slate-50">
                    <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center">
                      <Icon className="w-5 h-5 text-slate-700" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <Link href={`/magaza/${s.id}`} className="font-semibold hover:text-emerald-700 truncate">
                          {s.name}
                        </Link>
