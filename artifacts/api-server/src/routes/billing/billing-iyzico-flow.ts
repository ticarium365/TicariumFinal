/**
 * Billing provider flow (Iyzico + mock) — extracted from routes/billing.ts.
 *
 * Preserves:
 * - checkout / return / webhook behavior
 * - idempotency + payment state transitions
 * - analytics events
 */
...
import { findCreditPack } from "../../services/billing/credit-packs";
...
async function handleWebhookEvent(rawBody: string, headers: Record<string, string | string[] | undefined>) {
  ...
  if (IS_PRODUCTION && provider.name === "mock" && process.env.BILLING_ALLOW_MOCK_IN_PRODUCTION !== "true") {
    return {
      status: 503 as const,
      body: {
        error: {
          code: "BILLING_MOCK_DISABLED",
          message: "Üretimde mock ödeme sağlayıcısı kapalı. IYZICO_* ortam değişkenlerini ayarlayın veya acil durum için BILLING_ALLOW_MOCK_IN_PRODUCTION=true (önerilmez).",
        },
      },
    };
  }
...
export function registerBillingIyzicoFlow(router: Router): void {
  /* -------------------- POST /checkout -------------------- */
  router.post("/checkout", requireAuth, requireRole(["admin", "super_admin"]), async (req: Request, res: Response) => {
    try {
      if (IS_PRODUCTION && getBillingProvider().name === "mock" && process.env.BILLING_ALLOW_MOCK_IN_PRODUCTION !== "true") {
        return res.status(503).json({
          error: {
            code: "BILLING_MOCK_DISABLED",
            message: "Üretimde ödeme sağlayıcı yapılandırılmamış (IYZICO_*).",
          },
        });
      }
  ...
  router.post("/webhook", async (req: Request, res: Response) => {
    if (IS_PRODUCTION && getBillingProvider().name === "mock" && process.env.BILLING_ALLOW_MOCK_IN_PRODUCTION !== "true") {
      return res.status(503).json({
        error: {
          code: "BILLING_MOCK_DISABLED",
          message: "Üretimde billing webhook mock sağlayıcı ile kabul edilmez.",
        },
      });
    }
